...
async function directBuild(j,b){
  ...
  const classesJar=path.join(b,"classes.jar");
  await runCmd(j,"jar",["cf",classesJar,"-C",classes,"."],b);
  await runCmd(j,d8,["--lib",platform,"--output",dexout,classesJar],b);
...
app.post("/api/build",upload.fields([{name:"html",maxCount:1},{name:"icon",maxCount:1}]),async(req,res)=>{try{let h=req.files?.html?.[0];if(!h)return res.status(400).json({ok:false,error:"No HTML source received."});let id=crypto.randomBytes(8).toString("hex"),b=path.join(ROOT,id),perms=JSON.parse(req.body.permissions||"[]"),m={name:String(req.body.name||"My App").slice(0,70),pkg:pkg(req.body.pkg),version:/^\d+(\.\d+){0,3}$/.test(req.body.version||"")?req.body.version:"1.0.0",code:Math.max(1,parseInt(req.body.code)||1),orientation:["portrait","landscape","unspecified"].includes(req.body.orientation)?req.body.orientation:"portrait",min:[23,26,28].includes(Number(req.body.min))?Number(req.body.min):23,bg:col(req.body.bg,"#ffffff"),status:col(req.body.status,"#ffffff"),nav:col(req.body.nav,"#ffffff"),dark:req.body.dark==="true",zoom:req.body.zoom==="true",back:req.body.back!=="false",http:req.body.http==="true",permissions:perms,iconExt:(req.files?.icon?.[0]?.mimetype==="image/jpeg"?"jpg":req.files?.icon?.[0]?.mimetype==="image/webp"?"webp":"png"),project:"HtmlApkStudio"+id};let j={id,status:"queued",stage:"Queued",progress:4,logs:[],base:b,apk:null,name:m.name,pkg:m.pkg};jobs.set(id,j);const fallbackIcon=fs.readFileSync(path.join(__dirname,"public","default-icon.png"));await project(b,m,h.buffer.toString("utf8"),req.files?.icon?.[0]?.buffer||fallbackIcon);j.status="building";j.stage="Building APK";j.progress=18;log(j,"Android project generated.");log(j,"Package: "+m.pkg);directBuild(j,b).then(async a=>{await fsp.access(a);j.apk=a;j.status="done";j.stage="APK ready";j.progress=100;log(j,"BUILD SUCCESSFUL — APK created and signed.","success")}).catch(e=>{j.status="failed";j.stage="Build failed";log(j,"ERROR: "+(e.message||"Build failed"),"error")});res.json({ok:true,id,pkg:m.pkg})}catch(e){res.status(500).json({ok:false,error:e.message||"Server error"})}});
